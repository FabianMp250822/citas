...
// Importamos el hook de notificaciones hooks/ useNotifications.js
import useNotifications from "@/hooks/useNotifications.js";
// Opcional: Importamos el store de autenticación para obtener el id del agente
import useAuthenticationStore from "@/store/autenticationStore";

const NotificationMessage = () => {
  // Obtenemos el usuario para extraer el agentId (puede ser user.id, user.uid, etc.)
  const { user } = useAuthenticationStore();
  const agentId = user?.id || null;

  // Obtenemos las notificaciones del agente
  const notifications = useNotifications(agentId);

  // Calculamos el contador: si no hay notificaciones se mostrará 0
